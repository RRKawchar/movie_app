...
class MovieController extends GetxController{
  ...
  Future<void> fetchNowPlaying()async{
    try{
      var response=await ApiHandler.handleResponse(await ApiHandler.getRequest(apiUrl: ApiEndpoints.nowPlaying));
      if(response !=null){
        nowPlayings.value=[];
        for(var result in response['results']){
  ...
  Future<void> fetchUpComingMovie()async{
    try{
      var response=await ApiHandler.handleResponse(await ApiHandler.getRequest(apiUrl: ApiEndpoints.upcomingMovie));
      if(response !=null){
